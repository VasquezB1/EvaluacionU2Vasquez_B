package ec.edu.ups.EvaluacionU2Vasquez_B.EvaluacionU2Vasquez_BP59.test;
...
public class SampleIT {
    ...
    public static Archive<?> getEarArchive() {
        ...
        File f = new File("./target/EvaluacionU2Vasquez_BP59.war");
        if (f.exists() == false) {
            throw new RuntimeException("File " + f.getAbsolutePath() + " does not exist.");
        }
        WebArchive war = ShrinkWrap.create(ZipImporter.class, "EvaluacionU2Vasquez_BP59.war").importFrom(f).as(WebArchive.class);
        
        // Add the package containing the test classes:
        war.addPackage("ec.edu.ups.EvaluacionU2Vasquez_B.EvaluacionU2Vasquez_BP59.test");
